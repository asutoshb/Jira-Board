...
const calculateListPosition = async ({ projectId, status }: Issue): Promise<number> => {
  // Updated for TypeORM 0.3.x - find requires where clause
  const issues = await Issue.find({ where: { projectId, status } as any });

  const listPositions = issues.map((issue: Issue) => issue.listPosition);
